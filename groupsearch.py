...
for filename in os.listdir("./c04/"):
	...
	spacypro = subprocess.Popen([pythonbin, spacyscript, text], stdout=subprocess.PIPE, shell=False)
	...
	nounchunks = deleteDuplicates(nounchunks)
	...
	spacygroup = Counter(map(lambda x: x[0], spacyspan))

	#getting span for genia results
	geniarelevant = map(lambda x: x[0].lower(), geniarelevant)
	geniarelevant = deleteDuplicates(geniarelevant)
	index = 0
	...
	geniagroup = Counter(map(lambda x: x[0], geniaspan))

	allres = mergeLists(geniaspan, spacyspan)
	allgroup = Counter(map(lambda x: x[0], allres))

	final = open("./c04-results/" + filename, "w")
	...
	finalg = open("./c04-group/" + filename, "w")
	finalg.write("genia tagger: " + str(len(geniagroup.items())))
	for g in geniagroup.items():
		print>>finalg, g
	finalg.write("\n\nspacy + metamap: " + str(len(spacygroup.items())))
	for s in spacygroup.items():
		print>>finalg, s
	finalg.write("\n\ntotal: " + str(len(allgroup.items())))
	for a in allgroup.items():
		print>>finalg, a
	finalg.close()

	move = subprocess.call('mv ./c04/' + filename + " ../analysed/" , shell=True)